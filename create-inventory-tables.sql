-- =====================================================
-- Script de Criação das Tabelas de Estoque de Equipamentos
-- Execute este script no phpMyAdmin do banco onde2292_erp
-- =====================================================
...
CREATE TABLE IF NOT EXISTS `equipment_inventory` (
  ...
  CONSTRAINT `fk_equipment_user` FOREIGN KEY (`current_user_id`) REFERENCES `users` (`id`) ON DELETE SET NULL
...
CREATE TABLE IF NOT EXISTS `inventory_alerts` (
  ...
  KEY `created_at` (`created_at`),
  CONSTRAINT `fk_alert_equipment` FOREIGN KEY (`equipment_id`) REFERENCES `equipment_inventory` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Insere alguns equipamentos de exemplo (opcional)
INSERT INTO `equipment_inventory` (`serial_number`, `model`, `brand`, `type`, `location`, `purchase_date`, `purchase_price`) VALUES 
...
-- Mostra estatísticas
SELECT 
    (SELECT COUNT(*) FROM equipment_inventory WHERE status = 'available') as disponiveis,
    (SELECT COUNT(*) FROM equipment_inventory WHERE status = 'in_use') as em_uso,
    (SELECT COUNT(*) FROM equipment_inventory) as total;