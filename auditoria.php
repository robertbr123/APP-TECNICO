...
<body class="bg-background-light dark:bg-background-dark font-display">
        ...
        <div class="px-4 py-4 bg-white dark:bg-background-dark border-b border-gray-100 dark:border-gray-800">
            <div class="flex flex-col gap-3">
                ...
                <div class="flex gap-2">
                    <div class="flex-1 relative">
                        ...
                        <select id="filter-user"
                            class="w-full h-11 pl-11 pr-4 rounded-xl bg-gray-100 dark:bg-gray-800 text-[#111318] dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 appearance-none">
                ...
                <div class="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                    ...
                    <button class="filter-btn flex-shrink-0 h-9 px-4 rounded-full bg-gray-100 dark:bg-gray-800 text-[#111318] dark:text-white text-sm font-medium" data-action="new_client_page">
                        Acesso Cadastro
                    </button>
                    ...
                    <button class="filter-btn flex-shrink-0 h-9 px-4 rounded-full bg-gray-100 dark:bg-gray-800 text-[#111318] dark:text-white text-sm font-medium" data-action="link_equipment_page">
                        Acesso Vincular
                    </button>
    ...
    <script>
        ...
        function setupEventListeners() {
            ...
            filterUser.addEventListener('change', function(e) {
                currentFilters.user_id = e.target.value;
                loadAuditLogs();
            });

            // Busca por usuário (texto)
        ...
        async function loadAuditLogs() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    // Popula dropdown de usuários (apenas na primeira carga sem filtro de user)
                    if (result.data && result.data.length > 0 && !currentFilters.user_id) {
        ...
        function populateUserDropdown(logs) {
            ...
            logs.forEach(function(log) {
                ...
                if (userId && !usersMap[userId]) {
                    usersMap[userId] = log.user_full_name || log.username || 'Usuário ' + userId;
                }
            });

            // Só repopula se tiver novos usuários
            var existingCount = select.options.length;
            var newUsers = Object.keys(usersMap);
            ...
            newUsers.sort(function(a, b) {
                return usersMap[a].localeCompare(usersMap[b]);
            }).forEach(function(userId) {
                var option = document.createElement('option');
                option.value = userId;
                option.textContent = usersMap[userId];
        ...
        function renderLogCard(log) {
            ...
            if (log.details) {
                ...
                Object.keys(log.details).forEach(function(key) {
                    if (log.details[key]) {
                        ...
                        detailsHtml += '<p class="text-xs text-gray-500 dark:text-gray-400"><span class="font-medium">' + label + ':</span> ' + escapeHtml(log.details[key]) + '</p>';
                    }
                });
                detailsHtml += '</div>';
            }
            ...
            var entityInfo = '';
            if (log.entity_name) {
                entityInfo = '<p class="text-xs text-gray-500 dark:text-gray-400">' + escapeHtml(log.entity_name) + '</p>';
            }
                    ...
                    '<div class="flex-1 min-w-0">' +
                        '<div class="flex items-start justify-between gap-2">' +
                            '<div class="flex-1 min-w-0">' +
                                ...
                                '<p class="text-xs text-gray-600 dark:text-gray-400">' + escapeHtml(log.action_description) + '</p>' +
                                entityInfo +
                            '</div>' +
                        '</div>' +
                        detailsHtml +
                        '<div class="flex items-center gap-2 mt-2">' +
                            '<p class="text-xs text-gray-500 dark:text-gray-400">' + escapeHtml(userName) + '</p>' +
                            '<span class="text-gray-300 dark:text-gray-700">•</span>' +
                            '<p class="text-xs text-gray-500 dark:text-gray-400" title="' + escapeHtml(fullDate) + '">' + escapeHtml(timeAgo) + '</p>' +
                        '</div>' +
                    '</div>' +
                '</div>' +
            '</div>';
        ...
        function getActionIcon(actionType) {
            var icons = {
                'login': 'login',
                'new_client_page': 'add_circle',
                'client_created': 'person_add',
                'client_viewed': 'visibility',
                'link_equipment_page': 'devices',
        ...
        function getActionLabel(actionType) {
            var labels = {
                'login': 'Login',
                'new_client_page': 'Acesso - Página Cadastro',
                'client_created': 'Novo Cadastro',
                'client_viewed': 'Cliente Visualizado',
                'link_equipment_page': 'Acesso - Página Vincular',
